...
class Permintaan_mutasi extends CI_Controller {
	...
	{
        $this->data['js'] = 'transaksi/permintaan_mutasi_js';
        $this->data['main_view'] = 'transaksi/v_permintaan_mutasi';
    ...
    public function simpan()
    {
        ...
        if (isset($_POST['master']['no_pm'])) {
        	$url = BASE_URL_API_LPS.'gudang/umum/minta_mutasi/update';
        }else{
        	$url = BASE_URL_API_LPS.'gudang/umum/minta_mutasi/insert';
        }
    ...
    public function filter()
    {
        ...
        $headers  = getHeaderToken();
        $curl_handle = curl_init();
        curl_setopt($curl_handle, CURLOPT_URL, BASE_URL_API_LPS. 'gudang/umum/minta_mutasi/search');
        curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($curl_handle, CURLOPT_POST, 1);
        curl_setopt($curl_handle, CURLOPT_HTTPHEADER, $headers);
        curl_setopt($curl_handle, CURLOPT_POSTFIELDS,json_encode($data));
    ...
    public function hapus(){
        ...
        curl_setopt($curl_handle, CURLOPT_URL, BASE_URL_API_LPS.'gudang/umum/minta_mutasi/delete');
        curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($curl_handle, CURLOPT_POST, 1);
        curl_setopt($curl_handle, CURLOPT_HTTPHEADER, $headers);
    ...
    {
    	$API = BASE_URL_API_LPS."gudang/umum/minta_mutasi/get/".$_POST['data'];
    ...
    public function verifikasi()
    {
        ...
        curl_setopt($curl_handle, CURLOPT_URL, BASE_URL_API_LPS.'gudang/umum/minta_mutasi/status/'.$_POST['status']);
        curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($curl_handle, CURLOPT_POST, 1);
        curl_setopt($curl_handle, CURLOPT_HTTPHEADER, $headers);
        curl_setopt($curl_handle, CURLOPT_POSTFIELDS,json_encode($_POST['data']));
    ...
    public function penangungJawab()
    {
        ...
        curl_setopt($curl_handle, CURLOPT_URL, BASE_URL_API_LPS."gudang/umum/minta_mutasi/list_user_approve");
        curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($curl_handle, CURLOPT_POST, 1);
        curl_setopt($curl_handle, CURLOPT_HTTPHEADER, $headers);
        curl_setopt($curl_handle, CURLOPT_POSTFIELDS,json_encode($data));
    	...
    	// $API = BASE_URL_API_LPS."gudang/umum/minta_mutasi/list_user_approve";
        $daftar_unit = [];
        if ($result->metadata->list_count > 0) {
	        foreach ($result->list as $unit) {
	            $daftar_unit[] = (object) [
...
/* End of file Permintaan_mutasi.php */
/* Location: ./application/apps/general/controllers/transaksi/Permintaan_mutasi.php */