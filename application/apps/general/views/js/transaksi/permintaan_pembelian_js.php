...
<script type="text/javascript">
    ...
    var id = "<?=$id?>";
    $(function () {
      // $('#win-cari_barang_pu').window('open');
        // $(".dtb").val(setDateNow());
        if(id != ''){
          getData(id)
        }else{
    ...
    $('#btn-tutup_pu').click(function(event) {
      $('#txt-kriteria_barang_pu').val('');
      $('#win-cari_barang_pu').window('close');
    });

    $('#btn-tambah_pu').click(function () {
        var id_unit_asal = $('#cmb-data_unit option:selected').val();
        $('#txt-kriteria_barang_pu').val('');
        // console.log(id_unit_asal);
        if(id_unit_asal == "" || id_unit_asal == 0 || id_unit_asal == undefined)
    ...
    function filter_pu()
    {
        $('#dtg-cari_pu').datagrid('loadData',[]);
        var start_date = toAPIDateFormat($('#dtb-start_date_pu').val());
        var end_date = toAPIDateFormat($('#dtb-end_date_pu').val());
        var criteria = $('#txt-kriteria_barang_pu').val();
        var id_unit_asal = $('#cmb-data_unit option:selected').val();
        ...
        var dg = $('#dtg-cari_pu').datagrid({
          url : "<?php echo base_url("general/transaksi/permintaan_pembelian/filter_pu"); ?>",
          method: "POST",
          queryParams: data,
    ...
    function get_select()
    {
        // body...
        $.ajax({
            url : "<?php echo base_url("general/transaksi/permintaan_pembelian/get_data_unit"); ?>",
            type: "POST",
            dataType: 'json',
            beforeSend: function (){               
            },
    ...
    function filter()
    {
        ...
        // $.ajax({
        //   url : "<?php echo base_url("general/transaksi/permintaan_pembelian/filter"); ?>",
        //   type: "POST",
        //   dataType: 'json',
        //   data:{
        //     data: data,
    ...
    function filter_barang()
    {
        ...
        // $.ajax({
        //   url : "<?php echo base_url("general/transaksi/permintaan_pembelian/filter_barang"); ?>",
        //   type: "POST",
        //   dataType: 'json',
        //   data:{
        //     data: data,
    ...
    {
        $.ajax({
          url : "<?php echo base_url("general/transaksi/permintaan_pembelian/getPerKode"); ?>",
          type: "POST",
          dataType: 'json',
          data:{
            data: no_pp,
    ...
    $('#btn-pilih_barang').click(function(){
      ...
      var data_detail = $('#dtg-detail_item').datagrid('getRows');

      for (i=0;i<rows.length;i++)
      {
        rows[i]['id_item'] = rows[i]['id_item'];
        rows[i]['kd_item'] = rows[i]['kd_item'];
        rows[i]['nama_item'] = rows[i]['nama_item'];
        rows[i]['nama_satuan'] = rows[i]['id_satuan'];
        rows[i]['nama_kel_item'] = rows[i]['nama_kel_item'];
        rows[i]['jml_stok_all'] = 0,
        rows[i]['jml_stok'] = 0,
        rows[i]['jml_mutasi'] = 0,
        rows[i]['jml_minta'] = 1;
        rows[i]['tgl_kebutuhan'] = setDateNow();
        rows[i]['no_pm'] = "";

        data_detail.push(rows[i]);
      }

      $('#dtg-detail_item').datagrid('loadData', []);
      $('#dtg-detail_item').datagrid('loadData', data_detail);

      console.table(data_detail);

      $('#win-cari_data_item').window('close');
    });

    $('#btn-pilih_pu').click(function(){
      var rows = $('#dtg-cari_pu').datagrid('getSelections');
      ...
      for (i=0;i<rows.length;i++)
      {
        ...
        rows[i]['kd_item'] = rows[i]['kd_item'];
        rows[i]['nama_item'] = rows[i]['nama_item'];
        rows[i]['nama_satuan'] = rows[i]['nama_satuan'];
        rows[i]['nama_kel_item'] = rows[i]['nama_kel_item'];
        rows[i]['jml_stok_all'] = 0,
        rows[i]['jml_stok'] = 0,
        rows[i]['jml_mutasi'] = 0,
        rows[i]['jml_minta'] = rows[i]['jml'];
        rows[i]['tgl_kebutuhan'] = setDateNow();
        rows[i]['no_pm'] = rows[i]['no_pm'];
    ...
    $('#dtg-detail_item').datagrid({
      ...
      columns:[[
        ...
        {field:'nama_kel_item',title:'Jenis',width:150,halign:'center',align:'left'},
        {
          field:'jml_stok_all',
          title:'Stok All Depo',
          width:90,
        ...
        {field:'no_pm',title:'No. PU',width:150,halign:'center',align:'left'},
    ...
    function simpan()
    {
        ...
        $.ajax({
          url : "<?php echo base_url("general/transaksi/permintaan_pembelian/simpan"); ?>",
          type: "POST",
          dataType: 'json',
          data:{
            data: data,
    ...
    function hapus(no_pp)
    {
        ...
        swal.fire(cohapus()).then(function(result) {
            if (result.value) {
                $.ajax({
                  url : "<?php echo base_url("general/transaksi/permintaan_pembelian/hapus"); ?>",
                  type: "POST",
                  dataType: 'json',
                  data:{
                    data: data,
    ...
    function default_auth()
    {
        ...
        $.ajax({
          url : "<?php echo base_url("general/transaksi/permintaan_pembelian/default_auth"); ?>",
          type: "POST",
          dataType: 'json',
          data:{
            data: data,
    ...
    function verifikasi(data,status)
    {
      ...
      $.ajax({
        url : "<?php echo base_url("general/transaksi/permintaan_pembelian/status"); ?>",
        type: "POST",
        dataType: 'json',
        data:{
          data: data,
    ...
    function cetak()
    {
      ...
      console.log(data_get);
        ...
        $.ajax({
            url     : "<?= base_url() ?>general/transaksi/permintaan_pembelian/cetak",
            type    : "POST",
          dataType: 'json',	
            data:data_get,
          success:function(data, textStatus, jqXHR){
            if (data.success === true) {
              ...
              let file_cetak = "Laporan_Permintaan_Pembelian_General.pdf";
