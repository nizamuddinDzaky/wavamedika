...
class Permintaan_pembelian extends CI_Controller {
    ...
    public function Filter()
    {
        ...
        curl_setopt($curl_handle, CURLOPT_URL, BASE_URL_API_LPS.'gudang/farmasi/minta_beli/search');
    ...
    public function Filter_barang()
    {
        ...
        curl_setopt($curl_handle, CURLOPT_URL, BASE_URL_API_LPS.'gudang/farmasi/minta_beli/list_barang');
        curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($curl_handle, CURLOPT_POST, 1);
        curl_setopt($curl_handle, CURLOPT_HTTPHEADER, $headers);
        curl_setopt($curl_handle, CURLOPT_POSTFIELDS,json_encode($data));
    ...
    public function default_auth()
    {
        ...
        curl_setopt($curl_handle, CURLOPT_URL, BASE_URL_API_LPS.'gudang/farmasi/minta_beli/list_default_sign');
        curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($curl_handle, CURLOPT_POST, 1);
        curl_setopt($curl_handle, CURLOPT_HTTPHEADER, $headers);
    ...
    {
        $API = BASE_URL_API_LPS.'gudang/farmasi/minta_beli/get/'.$_POST['data'];
    ...
    public function get_data_unit()
    {
        ...
        curl_setopt($curl_handle, CURLOPT_URL, BASE_URL_API_LPS.'master/unit_akses/list_gudang_farmasi');
        curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($curl_handle, CURLOPT_POST, 1);
        curl_setopt($curl_handle, CURLOPT_HTTPHEADER, $headers);
        curl_setopt($curl_handle, CURLOPT_POSTFIELDS,json_encode($data));
    ...
    public function simpan()
    {
        ...
        if($_POST['edit']==0)
        {
            $API=BASE_URL_API_LPS.'gudang/farmasi/minta_beli/insert';
            $data['auths']=$_POST['data']['auths'];
        }
        else
        {
            $API=BASE_URL_API_LPS.'gudang/farmasi/minta_beli/update';   
        }
    ...
    public function cetak()
    {
        ...
        if ($param['type_file']==2)
        {
            ...
            header("Content-Disposition: attachment; filename=Laporan_Permintaan_Pembelian_Farmasi.xls");
        ...
        <div align="center"><b><u>LAPORAN PERMINTAAN PEMBELIAN Farmasi'; 
        $html.='</u></b><br>';
        $html.='</div><br>';
        ...
        if(count($detail)>0)
        {
            ...
            <tr style="border:1px black solid;">
                ...
                <th align="center" width="24%" colspan="6"><b>Nama Item</b></th>
                <th align="center" width="7%"><b>Satuan</b></th>
                <th align="center" width="10%"><b>Jenis</b></th>

                <th align="center" width="9%"><b>Stok All Depo</b></th>
                <th align="center" width="9%"><b>Stok</b></th>
                <th align="center" width="9%"><b>Pemakaian</b></th>

                <th align="center" width="9%"><b>Permintaan</b></th>
                <th align="center" width="9%"><b>Tgl. Kebutuhan</b></th>
            </tr>';
            $no=1;
            ...
            foreach ($detail as $key)
            {
                $html.='<tr style="border:1px black solid;">
                    ...
                    <th align="left" width="24%" colspan="6">'.$key['nama_item'].' </th>
                    <th align="left" width="7%">'.$key['nama_satuan']. '</th>
                    <th align="right" width="10%">'.$key['nama_kel_item'].'</th>

                    <th align="right" width="9%">'.angka($key['jml_stok_all'],0).'</th>
                    <th align="right" width="9%">'.angka($key['jml_stok'],0).'</th>
                    <th align="right" width="9%">'.angka($key['jml_mutasi'],0).'</th>
        ...
        else
        {
            ...
            $pdf->Output("assets/laporan/"."Laporan_Permintaan_Pembelian_Farmasi.pdf", "F");
            $return["success"] = TRUE;
