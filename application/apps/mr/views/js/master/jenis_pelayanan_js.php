<script type="text/javascript">
	...
	var title_datagrid = "Daftar Jenis Pelayanan";
	var id_datagrid    = "#dtg-jenis_pelayanan";
	...
	function btn_hapus()
    {
	  ...
	  index_grid = $(id_datagrid).datagrid("getRowIndex", row);
	  deleterow(index_grid,row.id_jpf);
	}
	...
	$(id_datagrid).datagrid({
      ...
      columns:[[
        {field:'id_jpf',title:'ID',width:'5%',halign:'center',align:'left',hidden:'true'},//
        {field:'nama_jpf',title:'Nama',width:'40%',halign:'center',align:'left',editor : {type:'textbox'}},
		{field:'jenis_jpf',title:'Jenis',width:'60%',halign:'center',align:'left',editor : {type:'textbox'}},
      ]],
	...
	function saverow()
    {
		...
		if (!nama.replace(/\s/g, '').length) {
			notif('warning','Nama tidak boleh kosong!');  
			return false;
		}
		...
		let data_simpan = {jenis_jpf:jenis,nama_jpf:nama};
		
		if (edit==1){
			
			data_simpan['id_jpf']=row.id_jpf;
		}
		simpan(data_simpan);
    }
	...
	function filter()
	{
		...
		$.ajax({
			url     : "<?php echo base_url("mr/master/Jenis_pelayanan/filter"); ?>",
			type    : "POST",
			dataType: 'json',
	...
	{
		$.ajax({
			url     : "<?php echo base_url("mr/master/Jenis_pelayanan/simpan"); ?>",
			type    : "POST",
			dataType: 'json',
			data    :{
				edit: edit,
	...
	{   
		let data = {id_jpf: id}
        $.ajax({
			url     : "<?php echo base_url("mr/master/Jenis_pelayanan/hapus"); ?>",
			type    : "POST",
			dataType: 'json',
			data    :{
				data: data,
