<script type="text/javascript">
	...
	var title_datagrid = "Daftar Jenis Pemeriksaan Radiologi";
	var id_datagrid    = "#dtg-pemeriksaan";
	...
	function btn_hapus()
    {
	  ...
	  index_grid = $(id_datagrid).datagrid("getRowIndex", row);
	  deleterow(index_grid,row.id_tarif);
	}
	...
	$(id_datagrid).datagrid({
      ...
      columns:[[
        {field:'id_tarif',title:'ID',width:'5%',halign:'center',align:'left',hidden:'true'},//
        {field:'uraian',title:'Nama Pemeriksaan',width:'30%',halign:'center',align:'left'},
        ...
        formatter:function(value,row){
            ...
            options:{
                ...
                valueField:'jenis',
                textField :'jenis',
                url       :'<?php echo base_url("mr/master/jenis_pemeriksaan_radiologi/list_jenis") ?>',
                required  :true
            }
            }
        },
        {field:'golongan',title:'Golongan',width:'20%',halign:'center',align:'left',
        formatter:function(value,row){
                return row.golongan || value;
            },
            editor:{
            type:'combobox',
            options:{
                ...
                valueField:'golongan',
                textField :'golongan',
                url       :'<?php echo base_url("mr/master/jenis_pemeriksaan_radiologi/list_golongan") ?>',
                required  :true
            }
            }
        },
        {field:'jenis_sampel',title:'Kategori',width:'20%',halign:'center',align:'left',
        formatter:function(value,row){
                return row.jenis_sampel || value;
            },
            editor:{
            type:'combobox',
            options:{
                ...
                panelMinHeight:50,
                panelMaxHeight:200,
                valueField:'jenis_sampel',
                textField :'jenis_sampel',
                url       :'<?php echo base_url("mr/master/jenis_pemeriksaan_radiologi/list_kategori") ?>',
                required  :true
            }
            }
        }
	...
	function saverow()
    {
        ...
        var golongan = $(golongan_ed.target).combobox('getValue');

    	var jenis_sampel_ed = $(id_datagrid).datagrid('getEditor', {
            index: index_grid,
            field: 'jenis_sampel'
        });
        var jenis_sampel = $(jenis_sampel_ed.target).combobox('getValue');
        ...
        if (!jenis_sampel.replace(/\s/g, '').length) {
		...
		let data_simpan = {jenis:jenis,golongan:golongan,jenis_sampel:jenis_sampel};

		if (edit==1){
			
			data_simpan['id_tarif']=row.id_tarif;
		}
	...
	function filter()
	{
		...
		$.ajax({
			url     : "<?php echo base_url("mr/master/jenis_pemeriksaan_radiologi/filter"); ?>",
			type    : "POST",
			dataType: 'json',
	...
	{
        $.ajax({
			url     : "<?php echo base_url("mr/master/jenis_pemeriksaan_radiologi/simpan"); ?>",
			type    : "POST",
			dataType: 'json',
			data    :{
				edit: edit,
	...
	function hapus(id)
	{   
		let data = {id_tarif: id}
        $.ajax({
			url     : "<?php echo base_url("mr/master/jenis_pemeriksaan_radiologi/hapus"); ?>",
			type    : "POST",
			dataType: 'json',
			data    :{
				data: data,
