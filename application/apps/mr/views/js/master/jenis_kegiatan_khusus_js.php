<script type="text/javascript">
	...
	var title_datagrid = "Daftar Jenis Kegiatan Khusus";
	var id_datagrid    = "#dtg-jenis_kegiatan_khusus";
	...
	function btn_hapus()
    {
	  ...
	  index_grid = $(id_datagrid).datagrid("getRowIndex", row);
	  deleterow(index_grid,row.id_jnskegiatankhusus);
	}
	...
	$(id_datagrid).datagrid({
      ...
      columns:[[
        {field:'id_jnskegiatankhusus',title:'ID',width:'5%',halign:'center',align:'left'},
        {field:'nama_kegiatan',title:'Jenis Pasien',width:'95%',halign:'center',align:'left',editor : {type:'textbox'}},
      ]],
	...
	function saverow()
    {
		var nama_kegiatan_ed = $(id_datagrid).datagrid('getEditor', {
            index: index_grid,
            field: 'nama_kegiatan'
        });
        var nama_kegiatan = $(nama_kegiatan_ed.target).textbox('getValue');

		if (!nama_kegiatan.replace(/\s/g, '').length) {
		...
		let data_simpan = {nama_kegiatan:nama_kegiatan};

		if (edit==1){
			
			data_simpan['id_jnskegiatankhusus']=row.id_jnskegiatankhusus;
		}
	...
	function filter()
	{
		...
		$.ajax({
			url     : "<?php echo base_url("mr/master/Jenis_kegiatan_khusus/filter"); ?>",
			type    : "POST",
			dataType: 'json',
	...
	{	
        $.ajax({
			url     : "<?php echo base_url("mr/master/Jenis_kegiatan_khusus/simpan"); ?>",
			type    : "POST",
			dataType: 'json',
			data    :{
				edit: edit,
	...
	function hapus(id)
	{   
		let data = {id_jnskegiatankhusus: id}
        $.ajax({
			url     : "<?php echo base_url("mr/master/Jenis_kegiatan_khusus/hapus"); ?>",
			type    : "POST",
			dataType: 'json',
			data    :{
				data: data,
