<script type="text/javascript">
	...
	var title_datagrid = "Daftar Jenis Pasien";
	var id_datagrid    = "#dtg-jenis_pasien";
	...
	function btn_hapus()
    {
	  ...
	  index_grid = $(id_datagrid).datagrid("getRowIndex", row);
	  deleterow(index_grid,row.id_jnspasien);
	}
	...
	$(id_datagrid).datagrid({
      ...
      columns:[[
        {field:'id_jnspasien',title:'ID',width:'5%',halign:'center',align:'left'},
        {field:'jenis_pasien',title:'Jenis Pasien',width:'95%',halign:'center',align:'left',editor : {type:'textbox'}},
      ]],
	...
	function saverow()
    {
		var jenis_pasien_ed = $(id_datagrid).datagrid('getEditor', {
            index: index_grid,
            field: 'jenis_pasien'
        });
        var jenis_pasien = $(jenis_pasien_ed.target).textbox('getValue');

		if (!jenis_pasien.replace(/\s/g, '').length) {
		...
		let data_simpan = {jenis_pasien:jenis_pasien};

		if (edit==1){
			
			data_simpan['id_jnspasien']=row.id_jnspasien;
		}
	...
	{	
        $.ajax({
			url     : "<?php echo base_url("mr/master/Jenis_pasien/simpan"); ?>",
			type    : "POST",
			dataType: 'json',
			data    :{
				edit: edit,
	...
	function hapus(id)
	{   
		let data = {id_jnspasien: id}
        $.ajax({
			url     : "<?php echo base_url("mr/master/Jenis_pasien/hapus"); ?>",
			type    : "POST",
			dataType: 'json',
			data    :{
				data: data,
