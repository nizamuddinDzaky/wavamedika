<script type="text/javascript">
	...
	var title_datagrid = "Daftar Jenis Darah";
	var id_datagrid    = "#dtg-jenis_darah";
	...
	function btn_hapus()
    {
	  ...
	  index_grid = $(id_datagrid).datagrid("getRowIndex", row);
	  deleterow(index_grid,row.kode_darah);
	}
	...
	$(id_datagrid).datagrid({
      ...
      columns:[[
		{field:'kode_darah',title:'Kode',width:'10%',halign:'center',align:'left',editor : {type:'textbox'}},
		{field:'jenis_darah',title:'Jenis Darah',width:'30%',halign:'center',align:'left',editor : {type:'textbox'}},
      ...
      onBeforeEdit:function(index,row){
		  var col = $(id_datagrid).datagrid('getColumnOption', 'kode_darah');
	...
	function saverow()
    {
		var kode_darah_ed = $(id_datagrid).datagrid('getEditor', {
            index: index_grid,
            field: 'kode_darah'
        });
		...
		if(edit==0)
		{
			var kode_darah = $(kode_darah_ed.target).textbox('getValue');

			if (!kode_darah.replace(/\s/g, '').length) {
				notif('warning','Kode Darah tidak boleh kosong!');  
				return false;
			}
		}

		var jenis_darah_ed = $(id_datagrid).datagrid('getEditor', {
            index: index_grid,
            field: 'jenis_darah'
        });
        var jenis_darah = $(jenis_darah_ed.target).textbox('getValue');

		if (!jenis_darah.replace(/\s/g, '').length) {
			notif('warning','Jenis Darah tidak boleh kosong!');  
			return false;
		}
		...
		if(edit==1){kode_darah=row.kode_darah}
		let data_simpan = {kode_darah:kode_darah,jenis_darah:jenis_darah,keterangan:keterangan};

		if (edit==1){
			
			data_simpan['kode_darah']=row.kode_darah;
		}
	...
	function filter()
	{
		...
		$.ajax({
			url     : "<?php echo base_url("mr/master/Jenis_darah/filter"); ?>",
			type    : "POST",
			dataType: 'json',
	...
	{
        $.ajax({
			url     : "<?php echo base_url("mr/master/Jenis_darah/simpan"); ?>",
			type    : "POST",
			dataType: 'json',
			data    :{
				edit: edit,
	...
	function hapus(id)
	{   
		let data = {kode_darah: id}
        $.ajax({
			url     : "<?php echo base_url("mr/master/Jenis_darah/hapus"); ?>",
			type    : "POST",
			dataType: 'json',
			data    :{
				data: data,
