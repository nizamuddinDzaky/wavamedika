<script type="text/javascript">
	...
	var title_datagrid = "Daftar Jenis Diagnosa";
	var id_datagrid    = "#dtg-jenis_diagnosa";
	...
	function btn_hapus()
    {
	  ...
	  index_grid = $(id_datagrid).datagrid("getRowIndex", row);
	  deleterow(index_grid,row.id_jnsdiagnosa);
	}
	...
	$(id_datagrid).datagrid({
      ...
      columns:[[
        {field:'id_jnsdiagnosa',title:'ID',width:'5%',halign:'center',align:'left',hidden:'true'},//
        {field:'jenis',title:'Jenis',width:'40%',halign:'center',align:'left',editor : {type:'textbox'}},
	...
	function saverow()
    {
		...
		let data_simpan = {jenis:jenis,keterangan:keterangan};

		if (edit==1){
			
			data_simpan['id_jnsdiagnosa']=row.id_jnsdiagnosa;
		}
	...
	function filter()
	{
		...
		$.ajax({
			url     : "<?php echo base_url("mr/master/Jenis_diagnosa/filter"); ?>",
			type    : "POST",
			dataType: 'json',
	...
	{
        $.ajax({
			url     : "<?php echo base_url("mr/master/Jenis_diagnosa/simpan"); ?>",
			type    : "POST",
			dataType: 'json',
			data    :{
				edit: edit,
	...
	function hapus(id)
	{   
		let data = {id_jnsdiagnosa: id}
        $.ajax({
			url     : "<?php echo base_url("mr/master/Jenis_diagnosa/hapus"); ?>",
			type    : "POST",
			dataType: 'json',
			data    :{
				data: data,
