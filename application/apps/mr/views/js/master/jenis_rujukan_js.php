<script type="text/javascript">
	...
	var title_datagrid = "Daftar Jenis Rujukan";
	var id_datagrid    = "#dtg-jenis_rujukan";
	...
	function btn_hapus()
    {
	  ...
	  index_grid = $(id_datagrid).datagrid("getRowIndex", row);
	  deleterow(index_grid,row.id_jnsperujuk);
	}
	...
	$(id_datagrid).datagrid({
      ...
      columns:[[
        {field:'id_jnsperujuk',title:'ID',width:'5%',halign:'center',align:'left',hidden:'true'},//
        {field:'nama_perujuk',title:'Nama Rujukan',width:'40%',halign:'center',align:'left',editor : {type:'textbox'}},
		{field:'jenis_rujukan',title:'Jenis Rujukan',width:'60%',halign:'center',align:'left',editor : {type:'textbox'}},
		{field:'kode_p',title:'Kode P',width:'5%',halign:'center',align:'left',hidden:'true'},
      ]],
	...
	function saverow()
    {
		var jenis_rujukan_ed = $(id_datagrid).datagrid('getEditor', {
            index: index_grid,
            field: 'jenis_rujukan'
        });
        var jenis_rujukan = $(jenis_rujukan_ed.target).textbox('getValue');

		var nama_perujuk_ed = $(id_datagrid).datagrid('getEditor', {
            index: index_grid,
            field: 'nama_perujuk'
        });
        var nama_perujuk = $(nama_perujuk_ed.target).textbox('getValue');

		if (!nama_perujuk.replace(/\s/g, '').length) {
			notif('warning','Nama Perujuk tidak boleh kosong!');  
			return false;
		}
		
		if (!jenis_rujukan.replace(/\s/g, '').length) {
			notif('warning','Jenis Perujuk tidak boleh kosong!');  
			return false;
		}

		console.log(jenis_rujukan.replace(/\s/g, '').length);
		if (jenis_rujukan.replace(/\s/g, '').length>10) {
			notif('warning','Jenis Perujuk terlalu panjang!');  
			return false;
		}
		
		$(id_datagrid).datagrid('endEdit', index_grid);
		var row = $(id_datagrid).datagrid('getRows')[index_grid];
		let data_simpan = {jenis_rujukan:jenis_rujukan,nama_perujuk:nama_perujuk,kode_p:''};

		if (edit==1){
			
			data_simpan['id_jnsperujuk']=row.id_jnsperujuk;
		}
	...
	function filter()
	{
		...
		$.ajax({
			url     : "<?php echo base_url("mr/master/Jenis_rujukan/filter"); ?>",
			type    : "POST",
			dataType: 'json',
	...
	{
        $.ajax({
			url     : "<?php echo base_url("mr/master/Jenis_rujukan/simpan"); ?>",
			type    : "POST",
			dataType: 'json',
			data    :{
				edit: edit,
	...
	function hapus(id)
	{   
		let data = {id_jnsperujuk: id}
        $.ajax({
			url     : "<?php echo base_url("mr/master/Jenis_rujukan/hapus"); ?>",
			type    : "POST",
			dataType: 'json',
			data    :{
				data: data,
