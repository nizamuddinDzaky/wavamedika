<script type="text/javascript">
	...
	var title_datagrid = "Daftar Tindakan Lain";
	var id_datagrid    = "#dtg-tindakan_lain";
	...
	function btn_hapus()
    {
	  ...
	  index_grid = $("#dtg-tindakan_lain").datagrid("getRowIndex", row);
	  deleterow(index_grid,row.id_jnstindakanlain);
	}
	...
	$(id_datagrid).datagrid({
      ...
      columns:[[
        {field:'id_jnstindakanlain',title:'ID',width:'5%',halign:'center',align:'left'},
        {field:'nama_tindakan',title:'Tindakan Lain',width:'95%',halign:'center',align:'left',editor : {type:'textbox'}},
        {field:'jenis',title:'jenis',width:'5%',halign:'center',align:'left',hidden:true},
      ]],
	...
	function saverow()
    {
		var tindakan_lain_ed = $(id_datagrid).datagrid('getEditor', {
            index: index_grid,
            field: 'nama_tindakan'
        });
        var tindakan_lain = $(tindakan_lain_ed.target).textbox('getValue');

		if (!tindakan_lain.replace(/\s/g, '').length) {
			notif('warning','Tindakan Lain tidak boleh kosong!');  
			return false;
		}
		
		$(id_datagrid).datagrid('endEdit', index_grid);
		var row = $(id_datagrid).datagrid('getRows')[index_grid];
		let data_simpan = {nama_tindakan:tindakan_lain};

		if (edit==1){
			
			data_simpan['id_jnstindakanlain']=row.id_jnstindakanlain;
		}
	...
	function filter()
	{
		...
		$.ajax({
			url     : "<?php echo base_url("mr/master/tindakan_lain/filter"); ?>",
			type    : "POST",
			dataType: 'json',
	...
	{	
        $.ajax({
			url     : "<?php echo base_url("mr/master/tindakan_lain/simpan"); ?>",
			type    : "POST",
			dataType: 'json',
			data    :{
				edit: edit,
	...
	function hapus(id)
	{   
		let data = {id_jnstindakanlain: id}
        $.ajax({
			url     : "<?php echo base_url("mr/master/tindakan_lain/hapus"); ?>",
			type    : "POST",
			dataType: 'json',
			data    :{
				data: data,
