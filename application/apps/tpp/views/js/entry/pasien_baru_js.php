<script type="text/javascript">
  ...
  const SimpanInsertPasien = () => {
    $("#insert-pasien").submit()
  }

  $("#insert-pasien").validate({
    rules: {
      nama_lengkap: {
        // lettersonly: true,
        required: true
    ...
    submitHandler: function(form) {
      SimpanData("#insert-pasien", "InsertPasien", "<?= base_url("tpp/entry/pasien_baru/ajax") ?>", function(respon) {
  ...
  function SimpanData(form, act, url, sukses = "") {
    $.ajax({
      type: "POST",
      url: "<?= base_url("tpp/entry/pasien_baru/") ?>" + act,
      data: $(form).serialize(),
      dataType: "JSON",
      beforeSend: function() {
        $.messager.progress({
