...
import dataJenisTransfusiDarahService from '../../../../services/master/jenis_transfusi_darah.service';
// import dataJenisTransfusiDarah, {detailDataJenisTransfusiDarah} from "../../../../pojo/master/jenis_transfusi_darah/data_jenis_transfusi_darah";
...
const MasterJenisTransfusiDarah = (props: IPropsMasterJenisTransfusiDarah) => {
    ...
    const getData = async (limit:any, page: any) => {
        try {
            ...
            const resp = await dataJenisTransfusiDarahService.datajenistransfusidarah(param, page);
            setData(resp.list);
    ...
    const onEditEnd = async (event: any) => {
        try {
            ...
            if(mode === 'add') {
                ...
                const resp = await dataJenisTransfusiDarahService.tambahjenistransfusidarah(payload);
                ...
                const payload = {
                    id_transfusi_darah: event?.row?.id,
                    nama: event?.row?.nama,
                    aktif: event?.row?.aktif,
                }
                const resp = await dataJenisTransfusiDarahService.ubahjenistransfusidarah(payload);
                if(resp.metadata.response_status === 200){
                    await getData(pageSize, pageNumber);
                    NotifySuccess(resp.metadata.message);
                } else {
    ...
    const removeData = async () => {
        try {
            setLoading(true);
            const payload = {
                id_transfusi_darah: selection.id,
            }
            const resp = await dataJenisTransfusiDarahService.hapusjenistransfusidarah(payload);      
            if(resp.metadata.response_status === 200){
                await getData(pageSize, pageNumber);
                NotifySuccess(resp.metadata.message);
            } else {
