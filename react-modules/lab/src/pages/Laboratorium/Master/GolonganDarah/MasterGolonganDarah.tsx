...
import dataGolonganDarahService from '../../../../services/master/golongan_darah.service';
...
const MasterGolonganDarah = (props: IPropsMasterGolonganDarah) => {
    ...
    const getData = async (limit:any, page: any) => {
        try {
            ...
            const resp = await dataGolonganDarahService.datagolongandarah(param, page);
            setData(resp.list);
    ...
    const removeData = async () => {
        try {
            setLoading(true);
            const payload = {
                id_golongan_darah: selection.id,
            }
            const resp = await dataGolonganDarahService.hapusgolongandarah(payload);
    ...
    const onEditEnd = async (event: any) => {
        try {
            ...
            if(mode === 'add') {
                ...
                const resp = await dataGolonganDarahService.tambahgolongandarah(payload);
                if(resp.metadata.response_status === 200){
                    await getData(pageSize, pageNumber);
                    NotifySuccess(resp.metadata.message);
                } else {
                ...
                const payload = {
                    id_golongan_darah: event?.row?.id,
                    nama: event?.row?.nama,
                    aktif: event?.row?.aktif,
                }
                const resp = await dataGolonganDarahService.ubahgolongandarah(payload);
                if(resp.metadata.response_status === 200){
                    await getData(pageSize, pageNumber);
                    NotifySuccess(resp.metadata.message);
                } else {
