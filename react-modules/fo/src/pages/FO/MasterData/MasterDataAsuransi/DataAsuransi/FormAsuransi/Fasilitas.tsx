...
import EasyUISelectFasilitas from '../../../../../../components/Shared/EasyUI/EasyUISelectFasilitas';
    ...
    return [
        {
            ...
            editor: ({row, error, rowIndex}: any) => {
                return (
                    <>
                        <EasyUISelectFasilitas
                            error={error}
                            value={row?.id_fasilitas}
                        />
                    </>
                )
            },
...
const Fasilitas: React.FC<IProps> = (props: IProps) => {
    ...
    const getData = async (limit?: number, page?: number ) => {
        try {
            ...
            const resp = await master_asuransiService.mkt_instansi_datafasilitasasuransi(payload);
        ...
        try {
            ...
            if(mode === 'add') {
                ...
                const payload = {
                    ...
                    id_fasilitas: event?.row?.nama_fasilitas
                }
                const resp = await master_asuransiService.mkt_instansi_insert_fasilitasasuransi(payload);
    ...
    const handleDelete = async () => {
        try {
            setLoading(true);
            const payload = {
                id_instansifasilitas: selection?.id_instansifasilitas
            }

            const resp = await master_asuransiService.mkt_instansi_delete_fasilitasasuransi(payload);
