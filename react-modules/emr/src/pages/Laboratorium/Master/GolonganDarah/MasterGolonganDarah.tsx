...
import {detailDataGolonganDarah} from "../../../../pojo/master/golongan_darah/data_golongan_darah";
import Metadata from "../../../../pojo/Metadata";
import {NotifySuccess} from "../../../../services/notification.service";
import {LinkButton, TextBox, Tooltip, NumberBox, ComboBox,CheckBox} from "rc-easyui";

const operators = ["A", "AB", "B", "O"];
...
const tindakanColumn = [
    ...
    {
        id: 'nama_golonganDarah',
        title: 'Nama Golongan Darah',
        width: '90%',
        align: 'left',
        editable: true,
    ...
    },
    {
        ...
        editor: ({ row,error }: any) => (
            <>
                <Tooltip content={error} tracking>
                    <CheckBox value={row?.aktif}></CheckBox>
                </Tooltip>
        ...
        render: ({row} : any) => (
                <>
                <div>
                    <CheckBox value={row?.aktif} checked={row?.aktif} disabled ></CheckBox>
                </div>
                </>
        ),
    }
...
interface IPropsMasterGolonganDarah {
    hidePagination?: boolean;
}

const MasterGolonganDarah = (props: IPropsMasterGolonganDarah) => {
    let {
        hidePagination
    } = props;
    ...
    const [data, setData] = useState<Array<detailDataGolonganDarah>>(dataDummy);
    const [meta, setMeta] = useState<Metadata>({});
    const [mode, setMode] = useState<string>('');
    const [selection, setSelection] = useState<detailDataGolonganDarah>({});
    ...
    const removeData = async () => {
        try {
            ...
            setData(newData);
            setLoading(false);
            NotifySuccess('Data Golongan Darah Berhasil Dihapus');
        ...
        try {
            ...
            if(mode === 'add') {
                ...
                const payload = {
                    ...
                    nama_golonganDarah: event?.row?.nama_golonganDarah,
                ...
                setMode('');
                setLoading(false);
                NotifySuccess('Data Golongan Darah Berhasil Diubah');
...
export default MasterGolonganDarah;
