...
import dataPTKPService from '../../../../services/master/ptkp.service';
// import {detailDataPTKP} from "../../../../pojo/master/PTKP/data_PTKP";
...
const tindakanColumn = [
    ...
    },
    {
        ...
        editor: ({ row,error }: any) => (
            <>
                <Tooltip content={error} tracking>
                    <NumberBox groupSeparator={'.'} value={row?.jumlah}/>
                </Tooltip>
            </>
        ),
        editRules: ['required'],
...
interface IPropsMasterPTKP {
    hidePagination?: boolean;
}

const MasterPTKP = (props: IPropsMasterPTKP) => {
    let {
        hidePagination
    } = props;
    ...
    const getData = async (limit:any, page: any) => {
        try {
            ...
            const resp = await dataPTKPService.dataptkp(param, page);
            setData(resp.list);
    ...
    const removeData = async () => {
        try {
            setLoading(true);
            const payload = {
                id_ptkp: selection.id,
            }
            const resp = await dataPTKPService.hapusptkp(payload);
    ...
    const onEditEnd = async (event: any) => {
        try {
            ...
            if(mode === 'add') {
                ...
                const resp = await dataPTKPService.tambahptkp(payload);
                if(resp.metadata.response_status === 200){
                    await getData(pageSize, pageNumber);
                    NotifySuccess(resp.metadata.message);
                } else {
                ...
                const payload = {
                    id_ptkp: event?.row?.id,
                    kode: event?.row?.kode,
                    jumlah: event?.row?.jumlah,
                    uraian: event?.row?.uraian,
                }
                const resp = await dataPTKPService.ubahptkp(payload);
                if(resp.metadata.response_status === 200){
                    await getData(pageSize, pageNumber);
                    NotifySuccess(resp.metadata.message);
                } else {
