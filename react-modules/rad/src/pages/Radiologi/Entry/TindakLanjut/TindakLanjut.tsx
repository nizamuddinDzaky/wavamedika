...
import EntryPajakKaryawan from './EntryTindakLanjut';
...
const TindakLanjut: React.FC<propTypes> = (props: propTypes) => {
    ...
    const dialogViewEntryTindakLanjut: any = useRef(null);

    const handleOpenTindakLanjut = () => {
            dialogViewEntryTindakLanjut?.current?.open()
    };
           ...
           <div className={'kt-portlet__head kt-portlet__head--lg kt-portlet__head--noborder kt-portlet__head--break-sm'}>
            ...
            <div className={'kt-portlet__body kt-portlet__body--fit kt-margin-b-20 kt-margin-t-20'}>
                <ListPasien
                    ...
                    onDoubleClickCell={() => dialogViewEntryTindakLanjut?.current?.open()}
                />
            </div>
            ...
            <CustomDialog
                ...
                // style={{height:'500px'}}
                ref={dialogViewEntryTindakLanjut}
            >
                {(dialogViewEntryTindakLanjut.current && select && select.nama_lengkap && select.sex && select.umur) &&
                    <EntryTindakLanjut
                    namaLengkap={select?.nama_lengkap!}
                    sexPasien={select?.sex!}
                    umurPasien={select?.umur!}
                    hidePagination={true}
                    />
                }
